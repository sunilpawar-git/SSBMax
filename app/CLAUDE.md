...
| DI modules | `di/{AppInjectablesModule,DatabaseModule,TestUseCaseModule,WorkManagerModule}.kt` | Koin bindings for the above — **not Hilt**; see [app/di/CLAUDE.md](di/CLAUDE.md) (`DebugModule` deleted in the KMP-convergence plan's Phase 9d — its `DebugConfig`/`BYPASS_SUBSCRIPTION_LIMITS` bypass briefly lived on as a Koin property, then was retired for good by the dev-subscription-override plan's Phase 6: `shared`'s `DeveloperSettings` + `SubscriptionOverride.FORCE_PREMIUM`, set from the Settings screen's Developer section on both platforms, replace it — no `BuildConfig`/Koin-property plumbing left in `app`) |
