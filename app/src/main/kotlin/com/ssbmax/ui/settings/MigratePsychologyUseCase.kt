...
/**
 * Use case for migrating Psychology topic and study materials to Firestore
 * Based on successful OIR and PPDT migration pattern
 */
class MigratePsychologyUseCase @Inject constructor(
    ...
    data class MigrationResult(
        ...
        val message: String
            get() = if (success) {
                "✓ Psychology migration successful! Migrated $materialsMigrated/$totalMaterials materials"
            } else {
                "⚠ Psychology migration completed with issues: $materialsMigrated/$totalMaterials materials migrated"
            }
    }
    ...
    suspend fun execute(): Result<MigrationResult> {
        return try {
            ...
            Log.d(TAG, "Starting Psychology migration...")
            ...
            // Step 2: Migrate study materials (8 materials: psy_1 to psy_8)
            ...
            val result = MigrationResult(
                success = topicMigrated && materialsMigrated == 8 && errors.isEmpty(),
    ...
    private suspend fun migrateTopicContent() {
        val topicInfo = TopicContentLoader.getTopicInfo("PSYCHOLOGY")
        
        val topicDocument = mapOf(
            "id" to "PSYCHOLOGY",
            "topicType" to "PSYCHOLOGY",
            ...
            "isPremium" to false,
            "metadata" to mapOf(
                "migratedBy" to "MigratePsychologyUseCase",
    ...
    private suspend fun migrateStudyMaterials(errors: MutableList<String>): Int {
        val materials = StudyMaterialsProvider.getStudyMaterials("PSYCHOLOGY")
        var successCount = 0
        ...
        for ((index, materialItem) in materials.withIndex()) {
            try {
                ...
                val materialDocument = mapOf(
                    ...
                    "version" to 1,
                    "lastUpdated" to System.currentTimeMillis(),
                    "tags" to listOf("Psychology Tests", "TAT", "WAT", "SRT", "Officer Like Qualities"),
                ...
                successCount++
                Log.d(TAG, "✓ Migrated material ${index + 1}/8: ${materialItem.id}")
    ...
    companion object {
        private const val TAG = "PsychologyMigration"
    }
}
