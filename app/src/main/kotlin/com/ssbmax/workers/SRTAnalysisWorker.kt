...
class SRTAnalysisWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            val submission = submissionRepository.getSRTSubmission(submissionId).getOrNull()
            if (submission == null) {
    ...
    private suspend fun generateAndAnalyze(submission: com.ssbmax.core.domain.model.SRTSubmission): Map<OLQ, OLQScore>? {
    ...
    private suspend fun validateOLQScores(
        submission: com.ssbmax.core.domain.model.SRTSubmission,
        olqScores: Map<OLQ, OLQScore>
    ) {
        val userProfile = try {
            userProfileRepository.getUserProfile(submission.userId).first().getOrNull()
        } catch (e: Exception) {
            ErrorLogger.log(e, "Failed to fetch user profile for SRT analysis — defaulting to NDA")
            null
        }
