 ...
 *
 * Triggered after user completes TAT test and submits all stories.
 * This worker:
 * 1. Fetches TAT submission from Firestore
...
class TATAnalysisWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        if (submissionId.isNullOrBlank()) {
            ...
            ErrorLogger.log(
                "TAT analysis worker started without submission ID",
                emptyMap(),
        ...
        Log.d(TAG, "🔄 Starting TAT analysis for submission: $submissionId")

        return try {
            // 1. Get submission from repository
            val submissionResult = submissionRepository.getTATSubmission(submissionId)
            val submission = submissionResult.getOrNull()
            if (submission == null) {
                Log.e(TAG, "❌ TAT submission not found: $submissionId")
                return Result.failure()
            }

            Log.d(TAG, "   Step 1: TAT submission found with ${submission.stories.size} stories")
            ...
            if (submission.analysisStatus != AnalysisStatus.PENDING_ANALYSIS) {
                Log.w(TAG, "⚠️ TAT submission not in PENDING_ANALYSIS state: ${submission.analysisStatus}")
            ...
            // 4. Generate TAT analysis prompt
            val prompt = PsychologyTestPrompts.generateTATAnalysisPrompt(submission)
            Log.d(TAG, "   Step 3: Generated TAT analysis prompt")
            ...
            val recommendations = listOf(
                "Continue practicing TAT with diverse scenarios",
            ...
            try {
                ...
                ErrorLogger.log(e, "Failed to send TAT result notification")
            }

            val durationMs = System.currentTimeMillis() - startTime
            Log.d(TAG, "🎉 TAT analysis completed successfully in ${durationMs}ms")
            ErrorLogger.log(
                "TAT analysis worker completed successfully",
                mapOf(
            ...
            val durationMs = System.currentTimeMillis() - startTime
            ErrorLogger.log(e, "TAT analysis worker failed for submission: $submissionId")
    ...
    /**
     * Analyze TAT submission with retry logic
     *
    ...
    private suspend fun handleAnalysisFailure(submissionId: String) {
        try {
            ...
            ErrorLogger.log(e, "Failed to update TAT submission status to FAILED")
        }
    }
}
