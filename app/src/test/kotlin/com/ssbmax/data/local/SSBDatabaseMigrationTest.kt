...
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_11_12
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_16_17
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_17_18
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_20_21
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_21_22
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_22_23
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_23_24
import com.ssbmax.data.local.DatabaseMigrations.MIGRATION_25_26
...
/**
 * `application = Application::class` (KMP-convergence Phase 9f, moved here
 * from the deleted `core:data` module): without it, Robolectric instantiates
 * `app`'s real manifest `Application` — `SSBMaxApplication` — whose
 * `onCreate()` calls `startKoin { modules(appModules) }` for real and then
 * resolves `BackgroundTaskScheduler` via `KoinJavaComponent.get()`, which
 * needs a real `WorkManager`/Android process Robolectric doesn't provide
 * here, confirmed by running this test unmodified and reading the real
 * `InstanceCreationException` it produced. This test only needs a raw
 * `Context` for `FrameworkSQLiteOpenHelperFactory`, not the app's DI graph,
 * so a bare `Application` sidesteps the bootstrap entirely rather than
 * joining the `@Ignore`d-for-SDK-35 tests elsewhere in this module.
 */
