...
class OIRTestViewModelTest : BaseViewModelTest() {
    ...
    fun setup() {
        ...
        // Mock submit use case — returns the session ID as the submission ID
    ...
    fun `submitTest calculates correct score and ends session`() = runTest {
        ...
        viewModel.uiState.test {
            ...
            // Verify submission was orchestrated via use case
    ...
    private val realScoreCalculator = OIRTestScoreCalculator(NoOpLogger())
    private val mockSubmitUseCase = mockk<SubmitOIRTestUseCase>(relaxed = true)
    private val mockTestSessionRepo = mockk<com.ssbmax.core.domain.repository.TestSessionRepository>(relaxed = true)
    private val mockContext = mockk<android.content.Context>(relaxed = true)
    private val fakeClock = com.ssbmax.time.FakeClock()

    private fun createViewModel(): OIRTestViewModel {
        return OIRTestViewModel(
            testContentRepository = mockTestContentRepo,
            testSessionRepository = mockTestSessionRepo,
