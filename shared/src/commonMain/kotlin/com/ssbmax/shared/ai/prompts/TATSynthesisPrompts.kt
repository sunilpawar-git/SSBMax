...
/**
 * KMP port of core:data's `TATSynthesisPrompts`. Deviates from the Android original
 * only in its input type: the Android worker cached per-story assessments as a Room
 * entity with `olqScoresJson: String` (parsed via `org.json.JSONArray`, Android-only);
 * [TATStoryAssessment] (the shared domain model backing this KMP port's in-memory
 * cache, see [com.ssbmax.shared.analysis.TATStoryAssessmentCache]) already stores
 * `olqScores: Map<OLQ, OLQScore>` directly, so no JSON round-trip is needed here.
 */
object TATSynthesisPrompts {

    fun buildPrompt(assessments: List<TATStoryAssessment>): String = buildString {
        ...
        assessments.sortedBy { it.storyIndex }.forEach { a ->
            ...
            appendLine("OLQ scores: ${a.olqScores.entries.joinToString(", ") { "${it.key.name}:${it.value.score}" }}")
        }
        appendLine()
