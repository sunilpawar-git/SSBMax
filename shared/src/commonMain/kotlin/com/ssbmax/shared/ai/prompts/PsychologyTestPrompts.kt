...
/**
 * KMP port of core:data's `PsychologyTestPrompts` (Android-only, `app`/`core:data`).
 * Straight port -- no Android dependencies in the original beyond the domain models,
 * which are already commonMain. WAT/SRT/SD only (TAT uses per-story +
 * [TATSynthesisPrompts] synthesis instead of a single-shot prompt on this path).
 */
