...
/**
 * KMP port of the Android `MockTestDataProvider` object. Same purpose and same
 * not-yet-removable status as the Android original (marked there for removal
 * once Firestore/cache always succeeds) -- [GitLiveTestContentRepository]
 * falls back to this data when the
 * relevant SQLDelight-backed cache manager fails, so it stays wired rather
 * than dead: candidates on a fresh install with a not-yet-synced cache or a
 * transient Firestore outage still see a usable test instead of a crash.
 *
 * Content is copied verbatim from the Android original with no Android
 * dependency to strip (it was already pure Kotlin domain-model construction).
 */
internal object TestContentMockFallback {

    fun ppdtQuestions(): List<PPDTQuestion> = listOf(
        PPDTQuestion(
            id = "ppdt_mock_1",
    ...
    fun tatQuestions(): List<TATQuestion> =
        (1..11).map { pos ->
            TATQuestion(
                id = "tat_mock_$pos",
    ...
    fun watWords(): List<WATWord> = listOf(
    ...
    fun srtSituations(): List<SRTSituation> = listOf(
        SRTSituation(
            id = "srt_mock_1",
    ...
    fun gpeQuestions(): List<GPEQuestion> = listOf(
        GPEQuestion(
            id = "gpe_generated_1",
