...
class SSBValidationCalibrationTest {
    ...
    fun `borderline candidate with one limitation scores DOUBTFUL`() {
        ...
        assertTrue(result.recommendation == Recommendation.DOUBTFUL || 
            result.recommendation == Recommendation.RECOMMENDED, "One critical limitation should cause concern")
    }
    ...
    fun `poor candidate exceeding NDA limitations scores NOT_RECOMMENDED`() {
        ...
        assertEquals(Recommendation.NOT_RECOMMENDED, result.recommendation, "NDA candidate with 5 limitations should be NOT_RECOMMENDED")
    }
    ...
    fun `OTA candidate with 8 limitations exceeds threshold`() {
        ...
        assertEquals(Recommendation.NOT_RECOMMENDED, result.recommendation, "OTA candidate with 8 limitations should be NOT_RECOMMENDED")
    }
    ...
    fun `Factor II average of 8 triggers auto-reject`() {
        ...
        assertTrue(criticalResult.hasAutoRejectWeakness, "Factor II average of 8 should trigger auto-reject")
    ...
    fun `Factor II average below 8 does not trigger auto-reject`() {
        ...
        assertFalse(criticalResult.hasAutoRejectWeakness, "Factor II average of 7 should NOT trigger auto-reject")
    }
    ...
    fun `Factor II average of 7_67 does not trigger auto-reject`() {
        ...
        assertFalse(criticalResult.hasAutoRejectWeakness, "Factor II average of 7.67 should NOT trigger auto-reject")
    }
    ...
    fun `Factor I consistency with 1 tick variation passes`() {
        ...
        assertTrue(factorIDetail!!.isConsistent, "Factor I with 1 tick variation should be consistent")
    }
    ...
    fun `Factor I consistency with 2 tick variation fails`() {
        ...
        assertNotNull(factorIDetail)
        assertFalse(factorIDetail!!.isConsistent, "Factor I with 2 tick variation should be inconsistent")
    }
    ...
    fun `Factor III consistency with 2 tick variation passes`() {
        ...
        assertTrue(factorIIIDetail!!.isConsistent, "Factor III with 2 tick variation should be consistent (allows ±2)")
    }
    ...
    fun `Factor III consistency with 3 tick variation fails`() {
        ...
        assertNotNull(factorIIIDetail)
        assertFalse(factorIIIDetail!!.isConsistent, "Factor III with 3 tick variation should be inconsistent")
    }
    ...
    fun `all 6 critical OLQs are correctly identified`() {
        ...
        assertEquals(6, criticalOLQs.size, "Should have exactly 6 critical OLQs")
    ...
    fun `non-critical OLQ at limitation is not flagged as critical weakness`() {
        ...
        assertFalse(criticalResult.criticalWeaknesses.contains(OLQ.EFFECTIVE_INTELLIGENCE), "EI is not a critical OLQ")
    }
    ...
    fun `NDA is stricter than OTA`() {
        ...
        assertNotEquals(Recommendation.NOT_RECOMMENDED, otaResult.recommendation, "OTA should not reject for 5 limitations alone")
    }
