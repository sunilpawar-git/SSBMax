 ...
 * - 4 study material documents
 *
 * Total: 5 Firestore writes
 *
...
class MigrateConferenceUseCase @Inject constructor(
    ...
    data class MigrationResult(
        ...
        val message: String
            get() = if (success) {
                "✓ Conference migration successful! Migrated $materialsMigrated/$totalMaterials materials"
            } else {
    ...
    suspend fun execute(): Result<MigrationResult> {
        return try {
            ...
            Log.d(TAG, "Starting Conference migration...")

            // Step 1: Migrate topic introduction via repository
            val topicMigrated = migrationRepository.migrateTopicContent("CONFERENCE")
                .onSuccess {
